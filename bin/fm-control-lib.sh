...
# Which task kinds an adapter is verified to run. muse and hermes are both
# crewmate/scout adapters only: neither has a primary supervision protocol,
# and bin/fm-spawn.sh refuses a --secondmate launch on either. The control
# plane asks this BEFORE it stops anything, so an incompatible relaunch
# target is refused while the current agent is still running rather than
# after it has been stopped.
fm_control_harness_supports_kind() {  # <harness> <kind>
  local harness=${1-} kind=${2-}
  fm_control_harness_supported "$harness" || return 1
  case "$harness" in
    muse|hermes) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}

# The key that cancels a running turn. Escape for every adapter except grok
# and hermes, whose Esc only moves focus (grok) or does nothing at all
# (hermes, verified live: a mid-tool-call Escape left the turn running
# unchanged); both cancel on Ctrl+C.
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # hermes's shared state.db DOES eventually record an interrupted turn
    # (a "[Command interrupted]" tool row plus a NULL-finish_reason assistant
    # row, verified live), but this verification did not measure how quickly
    # that write lands after the keystroke, the same open question cursor's
    # comment above raises about its own transcript. Claiming no immediate
    # acknowledgement here is the conservative choice until that latency is
    # measured.
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    hermes)
      # hermes installs no hook either: its busy source is its own shared
      # state.db, bound to the pane by this one firstmate-owned sidecar
      # (no "-current" cache file, unlike muse - the SQL resolution is cheap
      # enough to re-run on every classify). A relaunch AWAY from hermes must
      # retire it so no retired incarnation's session binding outlives the
      # agent.
