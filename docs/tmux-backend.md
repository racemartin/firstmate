...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, and Hermes process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
Hermes is the reverse case: `#{pane_current_command}` reports its venv's generic `python` interpreter, which no pattern here matches, so its exact self-renamed `hermes` kernel `comm` value - read first by the foreground probe - is sufficient alone with no structural fallback needed.
