...
hermes is likewise verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate: this verification did not investigate or build a primary watcher/turn-end supervision protocol for it, so there is no `docs/supervision-protocols/hermes.md`; [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md) owns its evidence.
