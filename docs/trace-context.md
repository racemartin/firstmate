...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, `muse`, and `hermes`, plus Secondmate spawns across that same set except the deliberately crewmate-only `muse` and `hermes` adapters.
